...
void sr_handleip(struct sr_instance* sr,
	...
	print_hdrs(packet,len+sizeof(sr_ethernet_hdr_t)); /* len doesn't include header */
	...
	int is_match=sr_checkinterfaces(inter,ip_header->ip_dst);
	if (is_match){
		...
		sr_find_dest(sr, ip_header->ip_dst,(uint8_t*)(ip_header), len-sizeof(struct sr_ethernet_hdr), ethernet_header->ether_dhost,interface);
	}
}
...
void sr_handlearp(struct sr_instance* sr,
	...
	if (ntohs(arp_header->ar_op)==arp_op_reply){
		...
		if (is_mine ==1){
			...
			unsigned char *mac = arp_header->ar_tha;
			uint32_t ip = arp_header->ar_tip;
			struct sr_arpreq *req = sr_arpcache_insert(cache, mac, ip);
			if (req != NULL) {
				struct sr_packet *req_pkt = req->packets;
				while(req_pkt != NULL) {
					struct sr_ethernet_hdr* ethernet_header=(struct sr_ethernet_hdr*)(req_pkt);
					sr_find_dest(sr, ip, req_pkt, req_pkt->len, inter->addr, interface);
...
void sr_find_dest(struct sr_instance* sr, uint32_t dest, uint8_t* packet, unsigned int len, uint8_t* shost, const char* interface){
	...
	int routing_match = sr_checkroutingtable(routing_table,dest);
	if (routing_match) {
